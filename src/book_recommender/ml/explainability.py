...
def _generate_summary(query_text: str, recommended_book: Dict[str, Any], contribution_scores: Dict[str, float]) -> tuple[str, List[str]]:
    """
    ...
    if contribution_scores.get('description_keywords', 0) > 0.1:
        if recommended_book.get('description'):
            ...
            if common_keywords:
                matching_features.append(f"has keywords in description like '{', '.join(common_keywords[:3])}'") # Limit to 3 keywords
...
def get_contribution_scores(query_text: str, recommended_book: Dict[str, Any]) -> Dict[str, float]:
    ...
    query_genres = set([g.strip() for g in query_text.lower().replace('genre', '').replace('genres', '').split() if g.strip()])
...
if __name__ == '__main__':
    ...
    print(f"  - Description Keywords Contribution: {explanation_result['details']['description_keywords_contribution']}")
    ...
    print(f"  - Description Keywords Contribution: {explanation_result_2['details']['description_keywords_contribution']}")
