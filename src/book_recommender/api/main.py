...
from book_recommender.api.models import RecommendByQueryRequest, RecommendByTitleRequest, RecommendationResult, Book, BookStats, BookSearchResult, BookCluster, ExplanationResponse, ExplainRecommendationRequest, FeedbackRequest, FeedbackStatsResponse
from book_recommender.api.dependencies import get_recommender, get_sentence_transformer_model, get_clusters_data
...
from book_recommender.core.logging_config import configure_logging # Import the logging configuration
import book_recommender.core.config as config
from book_recommender.ml.explainability import explain_recommendation
from book_recommender.ml.feedback import save_feedback, get_all_feedback
...
async def lifespan(app: FastAPI):
    ...
    if not IS_TESTING:
        ...
        try:
            ...
            logger.error(f"Failed to pre-load recommender or embedding model at startup: {e}")
            raise
    else:
...
@app.get("/health", summary="Perform a health check", response_description="Return HTTP Status Code 200 (OK)")
async def health_check():
    ...
    try:
        ...
        logger.error(f"Health check failed due to an issue with core services: {e}")
        raise HTTPException(
...
@app.post("/recommend/query", response_model=List[RecommendationResult], summary="Get book recommendations based on a natural language query")
async def recommend_by_query(
    request: RecommendByQueryRequest,
    ...
    """
    try:
        query_embedding = model.encode(request.query, show_progress_bar=False)
        ...
        logger.error(f"Error recommending by query: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal server error during recommendation.")


@app.post("/recommend/title", response_model=List[RecommendationResult], summary="Get similar books based on a book title")
async def recommend_by_title(
    request: RecommendByTitleRequest,
    recommender: BookRecommender = Depends(get_recommender)
):
    """
    ...
    try:
        recommendations = recommender.get_recommendations(
            request.title, top_k=request.top_k
        )
        if not recommendations:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Book with title '{request.title}' not found or no recommendations met the similarity threshold.")
        ...
        logger.error(f"Error recommending by title: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal server error during recommendation.")


@app.get("/books", response_model=BookSearchResult, summary="List all books with pagination")
...
@app.get("/books/search", response_model=BookSearchResult, summary="Search books by title or author with pagination")
async def search_books(
    recommender: BookRecommender = Depends(get_recommender),
    query: str = Query(..., min_length=2, description="Search query for title or author"),
    page: int = Query(1, ge=1, description="Page number"),
    page_size: int = Query(10, ge=1, le=100, description="Number of items per page")
):
    """
    ...
    try:
        ...
        mask = (all_books_df['title_lower'].str.contains(query.lower(), na=False)) | \
               (all_books_df['authors_lower'].str.contains(query.lower(), na=False))
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal server error while searching books.")


@app.get("/stats", response_model=BookStats, summary="Get database statistics")
async def get_stats(
    recommender: BookRecommender = Depends(get_recommender)
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal server error while fetching statistics.")

# --- Cluster Endpoints ---

@app.get("/clusters", response_model=List[BookCluster], summary="List all book clusters")
async def list_clusters(
    ...
    try:
        ...
        for cluster_id, name in cluster_names.items():
            ...
            all_clusters.append(BookCluster(
                id=cluster_id,
                name=name,
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal server error while listing clusters.")

@app.get("/clusters/{cluster_id}", response_model=BookSearchResult, summary="Get books in a specific cluster with pagination")
async def get_books_in_cluster(
    ...
    try:
        ...
        if cluster_id not in cluster_names:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Cluster with ID {cluster_id} not found.")
        ...
        logger.error(f"Error getting books in cluster {cluster_id}: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal server error while fetching books in cluster.")

@app.get("/clusters/{cluster_id}/sample", response_model=List[Book], summary="Get a random sample of books from a specific cluster")
async def get_cluster_sample(
    ...
    try:
        ...
        logger.error(f"Error getting sample from cluster {cluster_id}: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal server error while fetching cluster sample.")

@app.post("/explain", response_model=ExplanationResponse, summary="Get an explanation for a book recommendation")
async def explain_recommendation_endpoint(
    ...
    try:
        ...
        book_dict = request.recommended_book.model_dump()
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal server error during explanation generation.")

# --- Feedback Endpoints ---

@app.post("/feedback", status_code=status.HTTP_204_NO_CONTENT, summary="Submit user feedback on a recommendation")
async def submit_feedback(
    ...
    try:
        ...
        book_details_df = recommender.book_data[recommender.book_data['id'] == request.book_id]
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal server error while submitting feedback.")

@app.get("/feedback/stats", response_model=FeedbackStatsResponse, summary="Get aggregate feedback statistics")
async def get_feedback_stats():
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal server error while fetching feedback statistics.")