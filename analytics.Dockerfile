...
RUN groupadd -r analytics_user && useradd --no-create-home -r -g analytics_user analytics_user
...
CMD ["streamlit", "run", "src/book_recommender/apps/analytics_app.py", "--server.port=8502", "--server.address=0.0.0.0", "--server.runOnSave=false"]
