...
from src.book_recommender.api.dependencies import get_recommender, get_sentence_transformer_model, get_clusters_data # Import actual dependencies to override
...
class TestFastAPIEndpoints(unittest.TestCase):
    ...
    dummy_processed_data = pd.DataFrame({
        ...
        'description': ['Description for book 1', 'Description for book 2 with sci-fi themes', 'Another book by Author A', 'A magical adventure', 'Deep space exploration'],
        ...
        'combined_text': ['test book 1 by author a. genres: fiction. description: description for book 1. tags: ',
    ...
    def setUp(self):
        ...
        self.mock_recommender_instance = MagicMock(spec=BookRecommender) # Note: this is the instance returned by get_recommender
        ...
        self.mock_model.encode.side_effect = lambda x, **kwargs: np.array([np.random.rand(config.EMBEDDING_DIMENSION)]).astype('float32') if isinstance(x, str) else np.random.rand(len(x), config.EMBEDDING_DIMENSION).astype('float32')
        ...
        self.mock_recommender_instance.get_recommendations.return_value = [
            {'id': '3', 'title': 'Related Test Book 1', 'authors': 'Author A', 'description': 'Another book by Author A', 'genres': 'Fiction', 'similarity': 0.8},
            {'id': '1', 'title': 'Test Book 1', 'authors': 'Author A', 'description': 'Description for book 1', 'genres': 'Fiction', 'similarity': 0.7}
        ]
        self.mock_recommender_instance.get_recommendations_from_vector.return_value = [
            {'id': '5', 'title': 'Sci-Fi Classic', 'authors': 'Author D', 'description': 'Deep space exploration', 'genres': 'Science Fiction', 'similarity': 0.9},
            {'id': '2', 'title': 'Test Book 2', 'authors': 'Author B', 'description': 'Description for book 2 with sci-fi themes', 'genres': 'Science Fiction', 'similarity': 0.8}
        ]
        ...
        fastapi_app.dependency_overrides[get_recommender] = lambda: self.mock_recommender_instance # Return the instance
        fastapi_app.dependency_overrides[get_clusters_data] = lambda: self.mock_clusters_data_value # Return the actual value
    ...
    def test_health_check(self):
        ...
        self.assertEqual(response.json(), {"status": "OK", "message": "BookFinder API is healthy and core services are loaded."})
    ...
    def test_explain_recommendation_endpoint(self):
        ...
        response = self.client.post("/explain", json={
            "query_text": "A book about fiction",
            "recommended_book": sample_book,
